...
{
    public class FoodAuthHandler : AuthenticationHandler<FoodAuthOptions>
    {
        private readonly TokenStorage _tokenStorage;

        public FoodAuthHandler(IOptionsMonitor<FoodAuthOptions> options, ILoggerFactory logger, UrlEncoder encoder, 
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            if (!authorization.ToString().StartsWith("FoodAuth "))
            {
                return Task.FromResult(AuthenticateResult.NoResult());
            }

            // To delete scheme
            string token = authorization.ToString().Substring(9);
